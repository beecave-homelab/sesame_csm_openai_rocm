...
class DiaAdapter:
    ...
    def __init__(self, dia_model) -> None:
    ...
    def generate(
        ...
        try:
            ...
            if context and len(context) > 0:
                ...
                torchaudio.save(
                    audio_prompt_path, audio_prompt.unsqueeze(0).cpu(), self.sample_rate
                )
            ...
            if audio_prompt_path:
                ...
                logger.info("Generating with audio prompt from context")
                audio = self.dia_model.generate(
                    text=text, audio_prompt_path=audio_prompt_path
                )
            else:
                # Standard generation without voice cloning
                logger.info("Generating without audio prompt")
                audio = self.dia_model.generate(text)

            logger.info(
                f"Generation complete, got {len(audio) if audio is not None else 0} samples"
            )
            ...
            if (
                created_temp_file
                and audio_prompt_path
                and os.path.exists(audio_prompt_path)
            ):
                try:
